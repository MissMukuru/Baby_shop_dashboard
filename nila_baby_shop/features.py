...
@app.command()
def main(
    input_path: Path = PROCESSED_DATA_DIR / "baby_shop_demand_dataset_365_days.csv",
    ...
    df_forecast["lag_1_views"] = df_forecast.groupby("product")["views"].shift(1)
    df_forecast["lag_7_views"] = df_forecast.groupby("product")["views"].shift(7)

    df_forecast["rolling_mean_7"] = df_forecast.groupby("product")["views"].transform(
    ...
    df_viral["engagement"] = df_viral["views"] + df_viral["likes"] + df_viral["comments_count"]

    df_viral["engagement_growth"] = df_viral.groupby("product")["engagement"].diff().fillna(0)
